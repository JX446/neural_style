...
def loadmodel(model_file, pooling, use_gpu, disable_check):
    ...
    if "c" not in str(use_gpu).lower() or "c" not in str(use_gpu[0]).lower(): # c means cpu
        cnn = cnn.cuda()
    cnn = cnn.features # Use only the CNN's convolutional layers for feature extraction

    print_loadmodel(cnn, layerList)
